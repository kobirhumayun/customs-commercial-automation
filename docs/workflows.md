...
- for `export_lc_sc`, print planning must persist one checklist-source row-bundle record per selected workbook row; checklist generation and validation should prefer those persisted records over runtime reconstruction from mail outcomes. The HTML view should preserve workbook header text/order for the exported row fields and append mail-level `Mail Subject` plus `Document Filename`
...
- current phase-1 limitation: unlike `ud_ip_exp`, `export_lc_sc` still does not persist one checklist-source record per individual printed document. Its persisted checklist evidence is row-bundle based, so each export row currently carries the planned mail-group document set rather than an exact per-document-to-row binding.
- because of that weaker provenance model, `export_lc_sc` should still prefer hard-blocking over silent fallback whenever persisted export annotation records are missing, legacy fallback lineage cannot be reconciled, or workbook-row membership becomes uncertain.
