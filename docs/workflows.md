...
- for `export_lc_sc`, print planning must persist one checklist-source record per printed document; each persisted record must bind that printed PDF to the exact workbook `row_indexes` it supports, and checklist generation/validation should prefer those persisted records over runtime reconstruction from mail outcomes. The HTML view should preserve workbook header text/order for the exported row fields and append mail-level `Mail Subject` plus `Document Filename`
...
- because `export_lc_sc` now persists exact per-document row bindings, checklist validation should hard-block whenever persisted export annotation records are missing, saved-document lineage cannot be reconciled, or workbook-row membership becomes uncertain
