...
The `L/C Amnd No.` and `L/C Amnd Date` workbook fields use ERP `Amd No` and `Amd DT` respectively; if the ERP row value is blank, missing, or null, the workflow writes `-`.
The `Master L/C Recv. Date` workbook field always writes `-` for `export_lc_sc`.
The `Bangladesh Bank Ref.` workbook header and ERP `Ship. Remarks` report column are mandatory for export mapping. The ERP `Ship. Remarks` row value is trimmed and written only when the trimmed value contains at least 10 digits and every character is numeric. If `Ship. Remarks` is blank, null, fewer than 10 digits, or contains any nonnumeric character, the workflow skips only the `Bangladesh Bank Ref.` cell write and does not hard-block the mail for that field alone.
...
- UD extraction supports both standalone section tables and merged page-wide tables with spacer columns. In merged tables, locate LC and supplier headers within the table and use their populated column positions to read subsequent rows; never collapse empty data cells. Office-use-only number extraction may skip spacer cells on the same labeled row, stopping before `Date`. LC provenance retains the original physical row and value-column indexes. These layout adaptations do not change identifier matching, amendment value selection, or supplier quantity rules.
- In merged Base UD import-LC tables, a single-cell `Local` row is an in-table group label, like the existing `Foreign` label, rather than the end of the LC section. Continue reading the aligned LC rows beneath it while retaining the existing boundaries for other sections.
- If existing structured Amendment LC extraction is unresolved, support the observed headerless continuation grid on the immediately following page: physical columns `0, 1, 4, 5, 7, 9, 11` represent serial, LC identifier, date, value, increase/decrease, total value, and tolerance. Require a recognized preceding LC header, consecutive serials, empty spacer columns, and compatible date/money cells. Read only leading continuation rows, stopping at a subtotal, new section, or unexpected row. Preserve empty value cells and original page/row/column provenance; successful existing extraction and all business rules remain unchanged.
...
- A synthetic `mail_id` is derived from the normalized absolute path; the selected file SHA-256 is stored separately as integrity evidence.
- File Picker selections must resolve to regular `.pdf` files beneath the configured `import_document_root`. Duplicate selections after normalized absolute-path resolution are collapsed deterministically and reported.
...
- `Current Full Path`: every snapshotted mail receives an `import_relevance` record containing normalized subject, include hits, exclude hits, eligibility, and keyword revision
- `Current Full Path`: a subject-ineligible mail is reported with `processing_disposition=not_applicable`, `final_decision=pass` for backward schema compatibility, no discrepancies, and no write/print/mail-move eligibility; it remains in `working` and is excluded from action-outcome pass counts
- `Current Full Path`: acquire PDF bytes first into `run_artifact_root/import_btb_lc/<run_id>/source_documents/<mail_id>/<attachment_index>/`; after extraction validates the BTB LC date, copy to a same-directory temporary file under `<import_document_root>/<BTB LC year>/` and atomically rename it into place, leaving immutable source evidence intact
...
- BTB number normalization may trim outer whitespace, uppercase ASCII letters, normalize Unicode dash variants to `-`, and remove zero-width/control characters; it must not remove or repair internal whitespace, separators, or digits
...
- when no valid import BTB LC document remains after deterministic non-import exclusions, emit `import_required_document_missing`
...
- when seller PI extraction and BTB LC value extraction pass but another extraction field such as Related Export LC hard-blocks, the workflow must still validate all extracted PIs against the ERP import PI register for report evidence; display `Calculated Quantity (Kgs)` only when every extracted PI resolves to valid ERP rows and the aggregated ERP PI total amount exactly matches the extracted BTB LC value; otherwise keep the quantity cell blank and preserve the applicable PI discrepancy; this report-only validation never relaxes the extraction hard block or permits workbook writes
...
- subject-ineligible Current Full Path mails never move
...
- When dashboard LC Value is lower than ERP but dashboard quantity matches either ERP LC Qty (tolerance `0.01`) or ERP Net Weight (tolerance `0.8`), retain the value-failure warning and label it `Value mismatch`, adding independent mismatch topics as applicable. Detailed reasons retain the lower-value failure and identify the matching quantity reference. Matching net weight alone does not qualify for `OK (KGS)` when LC Value is lower. Other numeric cases and date-write behavior are unchanged.
...
- A returned dashboard `fetch_error` and a raised provider exception both stage the diagnostic status plus ERP shipment/expiry date refresh on eligible rows, while retaining `hard_block` as the family decision. ERP shipment and expiry dates must each resolve to one consistent, parseable calendar date before lookup. Missing, conflicting, or malformed family dates use the upstream ERP/input hard block with status-only writes. This parseability gate does not change the existing shipment/expiry comparison windows.
...
- Refresh the BB report after workbook write preparation/execution. Preserve family verification results and legacy planned-value fields, but report final `write_phase_status`, planned/confirmed operation counts, commit evidence, and write discrepancies separately. A committed state without matching commit evidence must not be labeled confirmed. HTML must explicitly distinguish planned cell values from confirmed writes, including no-write and uncertain outcomes.
- Decimal arithmetic errors in comparison evidence are reported as unavailable diagnostics while the family raw ERP/dashboard evidence and validation decision remain available. They must not abort report generation or processing of other families.
- Malformed/nonfinite dashboard numeric input is contained to the affected family with `bb_dashboard_numeric_input_invalid`, raw snapshot evidence, and diagnostic status write only; other families continue. Decimal arithmetic errors during comparison are contained the same way. Nonfinite ERP required totals use the existing upstream input hard block. Numeric parsing never accepts NaN or infinity.
- Otherwise eligible workbook rows without a usable LC family identifier remain unwritten and are listed in report `skipped_rows` with workbook row index, displayed `SL.No.`, raw LC value, and reason. This diagnostic list does not change family counts or candidate filtering.
- Reports preserve live workbook `SL.No.` display text, including leading zeros and number formatting. Numeric offline snapshot values retain their existing conversion to text. Date-writeback report fields describe only staged operations, not proof of a successful commit; they include both returned-error and raised-exception ERP date refreshes.
...
- `comparison_evidence.rule_results` is a report-only explanation of each check, with stable rule IDs, `pass`, `fail`, `unavailable`, or `not_applicable` status, prerequisites, and reasons. Independent checks remain visible when another input is invalid. Exact quantity, KGS, and excess are alternative acceptance paths; an unused path is not a family failure. Expiry ordering and the two shipment-to-expiry windows are separate checks. Missing/malformed inputs are distinguished from actual mismatches. Diagnostic `unavailable` results may expose existing validation weaknesses without replacing the authoritative family decision or legacy reasons.
- HTML comparison evidence follows decision summary, comparison table, then diagnostic explanations. The summary shows the authoritative family decision separately from a numeric acceptance path derived from `rule_results`; diagnostic findings use those same results rather than legacy numeric early-return reasons. Expandable rule references use the implementation constants. Additive `evaluation` text distinguishes alternatives that did not qualify from checks that were not required. Comparison rows link to rules, display readable lists and commodity totals, label ERP numeric references as aggregates, and distinguish non-applicable text differences from unavailable numeric/date differences. Currency and quantity units are not inferred. ERP and dashboard shipment-to-expiry intervals appear as separate rows. Legacy fields and decision/write behavior remain unchanged.
- Diagnostic rule failures are distinct from final mismatch topics. When lower LC Value accompanies a net-weight quantity match, retain the ERP LC Qty-specific diagnostic result and KGS ineligibility, but explain that net weight matches independently and the final quantity mismatch label is suppressed. HTML makes this distinction explicit. These explanation-only changes preserve rule IDs/statuses, acceptance decisions, report schema, and historical artifacts; new reports use the current rules without rewriting prior runs.
- Remaining investigation gaps: two unparseable LC dates compare equal after normalization to `None` (standard ERP providers reject invalid ERP LC dates upstream); dashboard settling requires both IRC/ERC although buyer validation permits one populated section. Dashboard list extraction discards blank cells consistently with the approved dashboard blank-skipping rule. No captured live example establishes how to detect an unfinished/paginated commodity table; this remains an investigation gap, not a new acceptance rule.
...
- each checklist row must also include `lc_sc`, `bangladesh_bank_ref`, and `ud_amendment_lc_value`
- the human-readable `ud_ip_exp` checklist table must render `UD/Amendment LC Value` between `Bangladesh Bank Ref.` and `SL.No.`
