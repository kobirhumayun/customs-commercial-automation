...
- For `ud_ip_exp`, an explicit attachment filename pattern `UD-LC-<suffix>-<buyer>` or `UD-SC-<suffix>-<buyer>` is supporting validation evidence only. When the ERP buyer name can be found in the filename, the suffix is the filename text between the explicit `UD-LC`/`UD-SC` prefix and the buyer-name boundary, allowing suffixes such as `1329-L`. If the buyer boundary is not found, legacy first-token suffix extraction is retained. The suffix must match the end of the ERP LC/SC number selected by the email-body file number lookup after separator-insensitive comparison; a mismatch hard-blocks as a likely renamed-against-wrong-family file. Filename suffixes must not be used to select the ERP row, workbook family, or UD LC table row.
...
| `import_btb_lc` | `import_amount` | `Amount` (column 22) | `update_if_blank` | row passed import LC candidate matching and BTB value validation; target blank at evaluation, staging, and live pre-write validation; never overwrite |
| `import_btb_lc` | `quantity_kgs` | `Quantity (Kgs)` | `update_if_blank` | row passed import LC candidate matching; extracted seller PIs resolved in ERP import PI register; aggregated ERP PI value exactly matches BTB LC value; target blank at evaluation, staging, and live pre-write validation; never overwrite |
...
- The structured UD/AM PDF LC table must match a row by ERP `Ship. Remarks` when found; if not found or blank, ERP `LC No.` may be used.
- ERP `Ship. Remarks` and ERP `LC No.` matching are exact first. If exact matching fails, the only permitted fallback is to strip zeros from the left side of the ERP/table identifier strings and compare those results. Leading and trailing spaces around compared values may be trimmed, but internal spaces must remain unchanged. No dashes, slashes, punctuation, internal zeros, or other characters may be modified.
...
- If both `warning` and `hard_block` are present for the same mail, hard-block takes precedence and no write/print/mail-move is allowed for that mail. `import_btb_lc` is the explicit write exception: independently writable BTB LC documents in that mail may still contribute staged writes, while the parent mail remains blocked from movement.
...
- PI-register validation is also required for report evidence when seller PI extraction and BTB LC value extraction pass but another required extraction field, including Related Export LC, hard-blocks. This report-only validation must not downgrade the extraction hard block, enable workbook row allocation, or produce staged writes.
- The HTML `Calculated Quantity (Kgs)` value is the aggregated ERP `Qty.Kg` and may be displayed only when every extracted PI resolves to valid ERP rows and the aggregated ERP PI `Total Amount` exactly equals the extracted BTB LC value. If the ERP report is unavailable or unreadable, any PI row is missing or invalid, or the aggregated amount differs from the BTB LC value, keep the HTML quantity cell blank and preserve the applicable PI discrepancy in the JSON/HTML report evidence.
- Only after that exact match may the aggregated ERP `Qty.Kg` be staged into workbook `Quantity (Kgs)`.
...
- Related export LC must pass the import LC/SC canonicalization profile and match workbook values by exact canonical equality. When an import LC clause omits the `LC` prefix, the extractor supplies that clause-defined prefix. Remove all whitespace from the identifier body, then strip all leading zero digits from that body; apply the same transformation to workbook `L/C & S/C No.` values. Preserve slash and dash separators and preserve the raw clause value and provenance unchanged. For example, `DC BD1184074` canonicalizes to `LC-DCBD1184074`, and `00012345` canonicalizes to `LC-12345`.
- An approved seller PI immediately followed by a date label without whitespace is deterministically bounded before that label; for example, `BTL/26/4271DATED` canonicalizes to `BTL/26/4271`.
- An authoritative related-export-LC clause may span two immediately adjacent inspected pages; the extractor joins only adjacent first-three-page LC-message evidence for this bounded clause match.
...
- BRAC also uses `ALL SHIPPING DOCUMENTS MUST BEAR THE EXPORT LC: <identifier> DATE/DATED ...`, without `NO.` or `NUMBER`. Accept this label within the same BRAC authoritative-clause rule, preserving the raw identifier (including leading zeros), existing canonicalization, and first-occurrence precedence over later sales-contract references.
...
- Import write eligibility is document-selective before run-level batch atomicity. A hard-blocked BTB LC document contributes no reservation or write, while independently validated BTB LC documents from the same mail retain their reservations and staged writes. The parent mail remains hard-blocked from movement.
- A same-run duplicate classification is valid only when its referenced primary document reached an accepted non-hard-block document outcome; parent-mail hard-block status does not invalidate that accepted document.
...
- The import workflow may write only `BTB L/C No.`, `BTB LC Issue Date`, import `Amount` column 22, and `Quantity (Kgs)`. Every other workbook cell is read-only evidence for import evaluation.
- The four import destination cells are append-only blank targets. All must remain blank at candidate evaluation, staged-plan construction, target prevalidation, and immediately before live batch apply. The workflow must never clear, append to, merge with, or replace a populated cell.
- Every staged operation for every import destination must use a canonical blank `expected_pre_write_value`. If a staged plan contains a non-blank expected value, the plan is invalid and hard-blocks before apply.
...
- Within the matching related-export-LC family, a row where only some import destination cells are populated among `BTB L/C No.`, `BTB LC Issue Date`, import `Amount` column 22, and `Quantity (Kgs)` is ineligible for import writes and must remain unchanged. Preserve the partial-target evidence in candidate reporting, but do not let that row hard-block selection of another fully blank eligible row in the same related-export-LC family.
...
- If any document in a mail hard-blocks, that document contributes zero writes. Other writable BTB LC documents in the same mail retain their selected rows and contribute writes; the parent mail remains hard-blocked and stays in `working`.
