...
- For the BRAC clause form `L/C NUMBER WITH DATE AND EXPORT SALES CONTRACT NO. <reference> <date-label>`, the first reference immediately following that combined label is the related export LC even when its body resembles a sales-contract identifier. Later `SALES CONTRACT NO.` or repeated contract references in the clause are not alternative related-export-LC candidates. The observed date labels `DATE`, `DATED`, `DAT`, and `DT` are valid deterministic boundaries for this extraction rule.
