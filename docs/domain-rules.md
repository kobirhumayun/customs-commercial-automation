...
- Within the matching related-export-LC family, a row where only some import destination cells are populated among `BTB L/C No.`, `BTB LC Issue Date`, import `Amount` column 22, and `Quantity (Kgs)` is an invalid partial target state and hard-blocks the affected import document.
