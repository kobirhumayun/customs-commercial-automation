...
- Implementation status:
  ...
  - live `ud_ip_exp` document reading is filename-gated: only PDFs beginning `UD-`, beginning `IP-`, or beginning with one or more digits followed by `-EXP` are processed; other PDFs are skipped before OCR/document analysis
