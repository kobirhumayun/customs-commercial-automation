...
- The persisted print plan should also carry checklist-source evidence sufficient for the active workflow: per-document annotation records for `ud_ip_exp`, and persisted export row-bundle annotation records for `export_lc_sc`.
...
- `annotation_documents` (array, may be empty): persisted checklist-source records in document print order. For `ud_ip_exp`, each record should include `saved_document_id`, `document_path_hash`, `document_filename`, `document_number`, `row_indexes`, and `checklist_required`. For `export_lc_sc`, each persisted record should represent one selected workbook row in print-plan order and should include `annotation_scope="export_row_bundle"`, `row_indexes`, `saved_document_ids`, `document_filenames`, `document_path_hashes`, and `checklist_required`.
- Current phase-1 implementation note: `export_lc_sc` checklist evidence is still weaker than `ud_ip_exp` evidence, but it should now be persisted in `annotation_documents` as explicit row-bundle records rather than reconstructed from mail outcomes at print time. That improves recovery fidelity and keeps checklist generation/validation anchored to the persisted print plan, while still falling short of `ud_ip_exp` on exact per-document-to-row provenance because each export row-bundle record carries the planned mail-group document set instead of one dedicated document binding per row.
