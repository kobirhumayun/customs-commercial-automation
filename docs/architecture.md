...
This model is intentionally **run-level staged, but mail-level selective**: one blocked mail must not force unrelated validated mails in the same run to be discarded, yet no single mail may print or move ahead of the controlled workbook-write phase. `import_btb_lc` is the explicit write-selection exception: each validated BTB LC document may contribute its own staged writes even when another BTB LC document in the same mail hard-blocks; the parent mail remains blocked from post-run movement.
...
- For `ud_ip_exp`, the human-readable HTML view also includes a diagnostic section below the unchanged Print Annotation Checklist named `Processed UD/IP/EXP PDF Documents`. This section is report-only: it must not add annotation requirements for supporting PDFs or change physical print order.
- The `ud_ip_exp` diagnostic section lists all UD/IP/EXP PDFs processed or evidenced by the run, including successful printed PDFs, successful duplicate/no-print PDFs whose destination filename already exists, and hard-blocked PDFs captured in discrepancy `document_evidence`. `Print Sequence` is the actual planned physical sequence when printed; otherwise it is `Not printed`.
- The `ud_ip_exp` diagnostic section should expose decision, disposition, raw extracted values, candidate evidence, and discrepancy evidence so operators can understand why each PDF wrote, skipped, printed, or hard-blocked. Where workbook `SL.No.` is displayed, it must use the workbook cell display text rather than row indexes or raw numeric values.
- A `ud_ip_exp` run that hard-blocks all processed mails before workbook write/print planning may still generate and open the HTML diagnostic report after terminal run handling, even though no print plan exists and the Print Annotation Checklist is empty.
...
- Explicit `UD-LC-<suffix>-<buyer>` or `UD-SC-<suffix>-<buyer>` filename evidence is a guardrail only: when the ERP buyer name is present in the filename, the suffix spans from the explicit LC/SC prefix to the buyer-name boundary, must agree with the ERP-derived LC/SC ending after separator-insensitive comparison, and must never replace email-body file-number plus ERP family resolution.
...
- Extraction is limited to the first three pages of the import BTB LC PDF and returns BTB LC number/date/value/currency, all distinct seller PI numbers, and the related export LC number from LC clauses.
- For phase-1 import BTB LC processing, every seller PI extracted from LC clauses must match one of the approved case-insensitive regex patterns: `btl/\d{2}/\d{4}` or `kyl/\d{2}/\d{4}`. Multiple valid PIs are preserved in deterministic document-occurrence order.
...
- Import writes are document-selective. Evaluate every BTB LC document independently in deterministic allocation order. A hard-blocked document contributes no writes, while another validated document from the same mail may retain its row reservation and staged writes. Same-run duplicate ownership and row reservations therefore follow surviving document outcomes rather than parent-mail outcome.
...
- If any import document in a mail is hard-blocked, the parent mail remains hard-blocked and cannot move, but each other validated import document in that mail may still contribute workbook writes to the run-level atomic batch.
