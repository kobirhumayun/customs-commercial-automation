...
- The persisted print plan should also carry checklist-source evidence sufficient for the active workflow: per-document annotation records for `ud_ip_exp`, and persisted per-document export annotation records for `export_lc_sc` that bind each printed PDF to the exact workbook row set it supports.
...
- `annotation_documents` (array, may be empty): persisted checklist-source records in document print order. For `ud_ip_exp`, each record should include `saved_document_id`, `document_path_hash`, `document_filename`, `document_number`, `row_indexes`, and `checklist_required`. For `export_lc_sc`, each persisted record should represent one printed PDF in print-plan order and should include `annotation_scope="export_document"`, `saved_document_id`, `document_path`, `document_path_hash`, `document_filename`, `row_indexes`, and `checklist_required`.
- `export_lc_sc` checklist generation and validation should aggregate those persisted per-document records back into workbook-row-oriented checklist rows so operator output remains row-centric while recovery and audit evidence stay document-exact.
