...
class ValidationTests(unittest.TestCase):
    ...
    def test_validate_run_snapshot_excludes_hard_blocked_mail_staged_writes_after_document_save_failure(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            workflow_year = __import__("datetime").datetime.now(
                tz=validate_timezone("Asia/Dhaka")
                    ...
                    [
                        {
                            ...
                            "sender_address": "one@example.com",
                            "body_text": "Please process file P/26/0042 today.",
                            "attachments": [{"attachment_name": "LC.pdf"}],
                        },
                        {
                            "entry_id": "entry-002",
                            "received_time": "2026-03-28T04:00:00Z",
            ...
            validation_result = validate_run_snapshot(
                ...
                attachment_content_provider=SimulatedAttachmentContentProvider(
                    content_by_key={(snapshot[1].entry_id, 0): b"%PDF-1.4\nok\n"}
                ),
            ...
            self.assertEqual(validation_result.mail_outcomes[0].saved_documents, [])
            self.assertEqual(validation_result.mail_outcomes[1].final_decision, FinalDecision.PASS)
            self.assertTrue(validation_result.mail_outcomes[1].eligible_for_write)
            self.assertTrue(validation_result.mail_outcomes[1].eligible_for_print)
            self.assertEqual(
                {operation.mail_id for operation in validation_result.staged_write_plan},
                {validation_result.mail_outcomes[1].mail_id},
            )
