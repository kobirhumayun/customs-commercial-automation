...
- Extract import LC number/date/value/currency, all distinct seller PI numbers, and related export LC; retrieve seller-PI quantities from the ERP import PI register and stage workbook `Quantity (Kgs)` only after aggregated PI value exactly matches the extracted BTB LC value.
