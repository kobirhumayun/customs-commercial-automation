...
- Extract import LC number/date/value/currency, all distinct seller PI numbers, and related export LC; retrieve seller-PI quantities from an operator-supplied ERP import PI register CSV/JSON and stage workbook `Quantity (Kgs)` only after aggregated PI value exactly matches the extracted BTB LC value. Live ERP report download remains deferred until the import ERP path is available for verification.
